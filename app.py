...
@keyframes glowPulse {
    0%,100% { box-shadow: 0 0 0 1px rgba(10,132,255,0.25), 0 0 18px rgba(10,132,255,0.08); }
    50%      { box-shadow: 0 0 0 1px rgba(10,132,255,0.55), 0 0 30px rgba(10,132,255,0.2); }
}
...
.hero {
    ...
    background:
        radial-gradient(ellipse 600px 400px at 5% 60%, rgba(10,132,255,.07) 0%, transparent 65%),
        radial-gradient(ellipse 450px 320px at 90% 10%, rgba(191,90,242,.05) 0%, transparent 60%),
...
.hero-eyebrow {
    ...
    padding: 4px 14px; border-radius: 100px; margin-bottom: 1.2rem;
}
.hero-h1 {
    font-size: 3.8rem; font-weight: 800; letter-spacing: -2.2px;
    line-height: .96; color: #F5F5F7; margin: 0 0 1rem;
}
.hero-sub {
    font-size: .95rem; font-weight: 300; font-style: italic;
    color: #86868B; line-height: 1.7; max-width: 420px; margin: 0 0 1.4rem;
}
.hero-stat {
    display: inline-flex; align-items: center; gap: 7px;
...
.live-dot { width: 7px; height: 7px; border-radius: 50%; background: #30D158; box-shadow: 0 0 6px rgba(48,209,88,.7); display:inline-block; }
...
.ctrl-label {
    ...
    text-transform: uppercase; color: #3A3A3C; margin-bottom: .7rem;
}
.ctrl-rule { border: none; border-top: 1px solid rgba(255,255,255,.05); margin: 1rem 0; }

/* ── thumb ── */
.qt { border-radius: 12px; overflow: hidden; border: 1px solid rgba(255,255,255,.07); position: relative; }
...
.qrow {
    ...
    border-radius: 14px; padding: 11px 16px; margin-bottom: 1.8rem;
}
.qrow-img { width: 48px; height: 64px; border-radius: 8px; overflow: hidden; border: 1px solid rgba(255,255,255,.08); flex-shrink: 0; }
.qrow-img img { width: 100%; height: 100%; object-fit: cover; display: block; }
.qrow-arr { color: #3A3A3C; }
.qrow-lbl { font-size: .58rem; font-weight: 600; letter-spacing: .14em; text-transform: uppercase; color: #3A3A3C; margin-bottom: 3px; }
...
/* ── misc ── */
.hrule  { border: none; border-top: 1px solid rgba(255,255,255,.05); margin: 0 0 2.5rem; }
...
col_hero, col_ctrl = st.columns([3, 1], gap="large")
...
with col_ctrl:
    ...
    if uploaded_file:
        ...
        enc = pil_b64(image)
        st.markdown(f'<div class="qt"><img src="{enc}"/><div class="qt-lbl">Your item</div></div>', unsafe_allow_html=True)
    ...
    free_text_pref = st.text_input("Other preferences", placeholder="e.g. avoid logos...")
...
# ── Full search ────────────────────────────────────────────────────────────────
if search_btn and uploaded_file:
    ...
    prog = st.empty()
    prog.markdown('<p style="padding:1rem 4rem;color:#3A3A3C;font-size:.82rem;">Removing background…</p>', unsafe_allow_html=True)
    candidates = retrieve_similar_items(temp_path, top_k=20)
    prog.markdown('<p style="padding:1rem 4rem;color:#3A3A3C;font-size:.82rem;">Applying preferences…</p>', unsafe_allow_html=True)
...
elif search_btn and not uploaded_file:
...
if st.session_state.results is None:
    st.markdown("""
    <div style="padding:3rem 4rem;color:#3A3A3C;font-size:.88rem;">
        <p style="color:#86868B;font-weight:500;margin-bottom:1rem;">Get started</p>
        <div style="display:flex;flex-direction:column;gap:.7rem;">
            <div style="display:flex;align-items:center;gap:10px;">
                <span style="width:22px;height:22px;border-radius:50%;background:#141414;border:1px solid rgba(255,255,255,.08);display:flex;align-items:center;justify-content:center;font-size:.6rem;font-weight:600;color:#86868B;flex-shrink:0">1</span>
                Upload a clothing image on the right
            </div>
            <div style="display:flex;align-items:center;gap:10px;">
                <span style="width:22px;height:22px;border-radius:50%;background:#141414;border:1px solid rgba(255,255,255,.08);display:flex;align-items:center;justify-content:center;font-size:.6rem;font-weight:600;color:#86868B;flex-shrink:0">2</span>
                Set your style preferences
            </div>
            <div style="display:flex;align-items:center;gap:10px;">
                <span style="width:22px;height:22px;border-radius:50%;background:#141414;border:1px solid rgba(255,255,255,.08);display:flex;align-items:center;justify-content:center;font-size:.6rem;font-weight:600;color:#86868B;flex-shrink:0">3</span>
...
notice = "" if (goals or avoid) else '<div class="notice">💡 No preferences active — select a style goal or avoid option to see personalized reranking.</div>'

base_grid   = '<div class="cgrid">' + "".join(base_card(i+1, r) for i, r in enumerate(results)) + '</div>'
rerank_grid = '<div class="cgrid">' + "".join(rerank_card(i+1, r, base_rank_map.get(r["image_path"])) for i, r in enumerate(reranked_results)) + '</div>'

st.markdown(f"""
<div class="sec-wrap">
    <div class="qrow">
        <div class="qrow-img"><img src="{qb}"/></div>
        <span class="qrow-arr">→</span>
        <div>
            <div class="qrow-lbl">Searching for</div>
    ...
    <div style="margin:2.5rem 0 2.5rem"><div class="hrule"></div></div>
